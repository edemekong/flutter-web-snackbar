...
class FlutterWebSnackbar<T> extends StatefulWidget {
  FlutterWebSnackbar(
      ...
      FlutterWebSnackDismissDirection dismissDirection =
          FlutterWebSnackDismissDirection.VERTICAL,
  ...
  /// [FlutterWebSnackDismissDirection.VERTICAL] by default.
  /// Can also be [FlutterWebSnackDismissDirection.HORIZONTAL] in which case both left and right dismiss are allowed.
  final FlutterWebSnackDismissDirection dismissDirection;
  ...
  final FlutterWebSnackStyle snackbarStyle;
  ...
  route.FlutterWebSnackbarRoute<T> _flushbarRoute;

  /// Show the snackbar. Kicks in [FlushbarStatus.IS_APPEARING] state followed by [FlushbarStatus.SHOWING]
  Future<T> show(BuildContext context) async {
    _flushbarRoute = route.showWebSnack<T>(
      context: context,
      snackbar: this,
    );
  ...
  /// Dismisses the snackbar causing is to return a future containing [result].
  ...
  /// Checks if the snackbar is visible
  bool isShowing() {
    return _flushbarRoute?.currentStatus == FlutterWebSnackStatus.SHOWING;
  }

  /// Checks if the snackbar is dismissed
  bool isDismissed() {
    return _flushbarRoute?.currentStatus == FlutterWebSnackStatus.DISMISSED;
  }

  @override
  State createState() {
    return _FlutterWebSnackbar<T>();
  }
}

class _FlutterWebSnackbar<K extends Object> extends State<FlutterWebSnackbar>
  ...
  Widget build(BuildContext context) {
    return Align(
      heightFactor: 1.0,
      child: Material(
        color: widget.snackbarStyle == FlutterWebSnackStyle.FLOATING
            ? Colors.transparent
            : widget.backgroundColor,
        child: SafeArea(
          minimum:
              widget.snackbarPosition == FlutterWebSnackPosition.BottomRight
                  ? EdgeInsets.only(
          ...
          bottom:
              widget.snackbarPosition == FlutterWebSnackPosition.BottomRight,
          top: widget.snackbarPosition == FlutterWebSnackPosition.TopRight,
          left: false,
          right: false,
          child: _getFlushbar(),
        ),
...
enum FlutterWebSnackDismissDirection { HORIZONTAL, VERTICAL }
...
enum FlutterWebSnackStatus { SHOWING, DISMISSED, IS_APPEARING, IS_HIDING }
